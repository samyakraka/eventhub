...
export function ChatBot() {
    ...
    try {
      ...
      if (
        ...
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "Sorry, I received an unexpected response from the AI. Please try again later.",
          },
        ]);
      }
    } catch (error) {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Sorry, I encountered an error communicating with the AI. Please ensure your Groqcloud API key is correct and try again.",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      <div
        className={`fixed bottom-4 right-4 w-[360px] h-[500px] bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl text-white flex flex-col overflow-hidden transition-all duration-300 ease-in-out z-50 ${
          isOpen
